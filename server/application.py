...
def kijiID2kijiBody(kiji_ID):
    ...
    URL = "https://www.nikkei.com/article/" + r.json()["hits"][0]["kiji_id_enc"]

    raw_body = r.json()["hits"][0]["body"]
    body_ja = list(filter(is_japanese, raw_body.split()))
    ...
    return {
        "body": body_ja,
        "title": list(filter(is_japanese, raw_title.split())),
        "link": URL,
        "sim": simlarities
    }
        ...
        lambda: jsonify(
            {
                ...
                "context": kijiID2kijiBody(request.get_json()["kiji_ID"]),
                "data": [
                    {"item": "lead", "ratio": 0.75},
                    {"item": "2st", "ratio": 0.64},
                    {"item": "3nd", "ratio": 0.23},
                    {"item": "4rd", "ratio": 0.56},
                    {"item": "5th", "ratio": 0.61},
        ...
        lambda: jsonify(
            {
                ...
                "test": kijiBody2similarity(request.get_json()["kiji_body"]),
            }
        )
    ),
    methods=["POST"],
